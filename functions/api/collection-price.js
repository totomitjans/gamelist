...
function isoDate(value) { const textValue = String(value || "").trim(); if (!textValue || /^\d{4}$/.test(textValue)) return ""; const leadingDate = textValue.match(/^\d{4}-\d{2}-\d{2}/)?.[0]; if (leadingDate) return leadingDate; const date = new Date(`${textValue} UTC`); return Number.isNaN(date.getTime()) ? "" : date.toISOString().slice(0, 10); }
