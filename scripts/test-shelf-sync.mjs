...
assert.match(sharedCss, /@media \(max-width: 760px\)[\s\S]*?\.board\s*\{[\s\S]*?display:\s*flex;[\s\S]*?transform: translateX\(calc\(var\(--mobile-section-index, 0\) \* -100%\)\);[\s\S]*?body\[data-mobile-section\] \.board \.column\.is-mobile-pane:not\(\[hidden\]\)\s*\{[\s\S]*?flex:\s*0 0 100%;/, "Main mobile board panes must slide horizontally in a stable flex track");
...
assert.match(shelfCss, /\.shelf-dialog #detailChips \.owner-pill,[\s\S]*?\.shelf-page \.game-row-core \.owner-pill\s*\{[\s\S]*?min-height:\s*18px;[\s\S]*?font-size:\s*10px;/, "Shelf detail and row owner tags must stay compact");
