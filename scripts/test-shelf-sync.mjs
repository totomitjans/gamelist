...
assert.match(sharedCss, /\.page-pull-switch\.is-dragging\s*\{[\s\S]*?transition:\s*none;[\s\S]*?\.page-pull-switch::before[\s\S]*?height:\s*35px;[\s\S]*?background:\s*var\(--accent\);[\s\S]*?transform:\s*translateX\(-50%\) scaleX\(0\.42\) scaleY\(0\.29\);[\s\S]*?\.page-pull-switch span[\s\S]*?font-size:\s*13px;[\s\S]*?\.page-pull-switch:hover::before[\s\S]*?background:\s*var\(--accent\);[\s\S]*?transform:\s*translateX\(-50%\) scaleX\(1\) scaleY\(1\);/, "Pull switch must use a pure accent 10px collapsed/35px expanded handle and smooth direct dragging");
...
assert.match(sharedCss, /@media \(max-width: 760px\)[\s\S]*?\.page-pull-switch\s*\{[\s\S]*?width:\s*min\(340px, calc\(100vw - 20px\)\);[\s\S]*?height:\s*76px;/, "Mobile pull handle must keep a larger grab target than the visible collapsed pill");
