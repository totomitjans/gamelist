...
assert.match(shelfCss, /\.shelf-dialog \.detail-cover\s*\{[\s\S]*?height:\s*auto;[\s\S]*?line-height:\s*0;[\s\S]*?font-size:\s*0;[\s\S]*?border-radius:\s*8px;[\s\S]*?\.shelf-dialog \.detail-cover img\s*\{[\s\S]*?display:\s*block;[\s\S]*?height:\s*auto;[\s\S]*?border-radius:\s*inherit;/, "Shelf detail cover must shrink-wrap the image so bottom corners clip exactly");
...
assert.match(shelfCss, /@media \(max-width: 760px\)[\s\S]*?\.shelf-dialog \.detail-cover\s*\{[\s\S]*?height:\s*auto;[\s\S]*?\.shelf-dialog \.detail-cover img\s*\{[\s\S]*?height:\s*auto;[\s\S]*?object-fit:\s*initial;/, "Shelf mobile detail cover must shrink-wrap the rendered image height");
...
assert.match(sharedCss, /\.settings-theme-card,[\s\S]*?--settings-control-height:\s*44px;[\s\S]*?\.settings-sync-card \.settings-visible-check\s*\{[\s\S]*?min-height:\s*var\(--settings-control-height\);[\s\S]*?\.settings-theme-select select\s*\{[\s\S]*?min-height:\s*var\(--settings-control-height, 44px\);/, "Settings check controls must match Theme and Default order dropdown height");
...
assert.match(collectionPriceSource, /const exactCandidate = rawCandidates\.find\(\(item\) => requestedId && item\.productId === requestedId\)[\s\S]*?const candidates = exactCandidate \? \[\] : filterVideoGameCandidates\(rawCandidates, query\)[\s\S]*?const candidate = requestedUrl \? \{ url: requestedUrl, productId: requestedId \} : idCandidate \|\| exactCandidate \|\|/, "PriceCharting public fallback must prefer the exact saved product ID before platform-filtered title matching");
