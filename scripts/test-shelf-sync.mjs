...
assert.match(shelfSource, /if \(frame\.classList\.contains\("detail-cover"\)\) \{[\s\S]*?frame\.style\.width = `\$\{width\}px`;[\s\S]*?frame\.style\.height = `\$\{renderedHeight\}px`;[\s\S]*?image\.style\.height = `\$\{renderedHeight\}px`;/, "Shelf detail cover must set real inline wrapper and image dimensions after the cover loads");
...
assert.match(sharedCss, /\.page-pull-switch[\s\S]*?touch-action:\s*none;[\s\S]*?\.page-pull-switch:hover::before[\s\S]*?width:\s*156px;[\s\S]*?height:\s*36px;[\s\S]*?\.page-pull-curtain[\s\S]*?transform:\s*translateY\(calc\(-102% \+ var\(--pull-distance, 0px\)\)\);[\s\S]*?\.page-pull-preview[\s\S]*?filter:\s*blur\(var\(--pull-blur, 18px\)\);[\s\S]*?body\.page-switching \.page-pull-curtain[\s\S]*?transform:\s*translateY\(0\);/, "Shared CSS must provide a draggable pull-down page transition with an expanding handle and blurred destination preview");
...
assert.match(sharedCss, /@media \(max-width: 760px\)[\s\S]*?\.topbar\s*\{[\s\S]*?padding:\s*28px 14px 12px;[\s\S]*?body\.page-pull-hover \.topbar,[\s\S]*?body\.page-pulling \.topbar\s*\{[\s\S]*?transform:\s*translateY\(12px\);/, "Mobile topbar must leave room for the pull-down handle and move down when its label is showing");
